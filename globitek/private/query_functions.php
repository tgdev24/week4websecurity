<?php
  ...
  function update_country($country) {
    ...
    $stmt->bind_param("ssi", $country['name'], $country['code'], $_GET['id']);
    $stmt->execute();
    $stmt->close();
  ...
  function update_state($state) {
    ...
    $stmt = $db->prepare("UPDATE states SET name=?, code=? WHERE id=?");
    $stmt->bind_param("ssi", $state['name'], $state['code'], $_GET['id']);
    $stmt->execute();
    $stmt->close();
  ...
  function insert_territory($territory) {
    ...
    $stmt = $db->prepare("INSERT INTO territories (name, state_id, position) VALUES (?, ?, ?)");
    $stmt->bind_param("sii", $territory['name'], $territory['state_id'], $territory['position']);
    $stmt->execute();
    //$id = mysqli_insert_id($dbc);
    $stmt->close();
  ...
  function update_territory($territory) {
    ...
    $stmt = $db->prepare("UPDATE territories SET name=?, position=? WHERE id=?;");
    $stmt->bind_param("ssi", $territory['name'], $territory['position'], $_GET['id']);
    $stmt->execute();
    $stmt->close();
  ...
  function update_salesperson($salesperson) {
    ...
    $stmt = $db->prepare("UPDATE salespeople SET first_name=?, last_name=?, phone=?, email=? WHERE id=? ;");
    $stmt->bind_param("ssssi", $salesperson['first_name'], $salesperson['last_name'], $salesperson['phone'], $salesperson['email'], $_GET['id']);
    $stmt->execute();
    $stmt->close();
  ...
  function update_user($user) {
    ...
    $stmt = $db->prepare("UPDATE users SET first_name=?, last_name=?, email=?, username=? WHERE id=? ;");
    $stmt->bind_param("ssssi", $user['first_name'], $user['last_name'], $user['email'], $salesperson['username'], $_GET['id']);
    $stmt->execute();
    $stmt->close();
